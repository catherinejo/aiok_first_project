...
2. **릴리즈 노트**: 자동 릴리즈 노트 생성 → release_notes 에이전트
...
PR_FETCHER_INSTRUCTION = """당신은 GitHub PR 정보를 수집하는 에이전트입니다.

사용자가 요청한 PR 번호에 대해 GitHub MCP 도구를 사용하여 다음 정보를 수집하세요:
1. PR 제목과 설명
2. 변경된 파일 목록
3. diff (코드 변경사항)
4. 커밋 목록
...
PR_ANALYZER_INSTRUCTION = """당신은 코드 변경사항을 분석하는 전문가 에이전트입니다.
...
PR_REVIEWER_INSTRUCTION = """당신은 시니어 개발자로서 PR 리뷰 포인트를 생성하는 에이전트입니다.
...
PR_COLLECTOR_INSTRUCTION = """당신은 GitHub에서 머지된 PR 목록을 수집하는 에이전트입니다.
